...
import { getRoutesFromDirectionsAPI, geocodeAddress, searchPlaces } from '../utils/directionsService'
import { calculateSafetyScore } from '../utils/safetyScore'
import { shareLocation } from '../utils/locationService'
import { updateUserLocationInDB, getUserLocationFromDB } from '../utils/backendLocationService'
import { 
  detectDevice, 
  getDeviceLocation, 
  watchDeviceLocation, 
...
import EmergencyContactSetup from './EmergencyContactSetup'
...
const JourneyPlanner = () => {
  ...
  const [showEmergencyContactSetup, setShowEmergencyContactSetup] = useState(false)
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      // Check if emergency contact is set (check backend database, not just localStorage)
      try {
        const response = await fetch(`${API_BASE_URL}/api/check-emergency-contact?uid=${currentUser.uid}`)
        let data = {}
        try {
          data = await response.json()
        } catch (parseError) {
          console.error('❌ Failed to parse emergency contact response:', parseError)
          data = { has_emergency_contact: false }
        }
        
        if (!data.has_emergency_contact) {
          // No emergency contact set in database, show modal
          const hasShownEmergencySetup = sessionStorage.getItem(`emergencySetupShown_${currentUser.uid}`)
          if (!hasShownEmergencySetup) {
            console.log('📱 No emergency contact found, showing setup modal')
            setShowEmergencyContactSetup(true)
            sessionStorage.setItem(`emergencySetupShown_${currentUser.uid}`, 'true')
          }
        } else {
          console.log('✅ Emergency contact already set:', data.emergency_contact_number)
        }
      } catch (error) {
      ...
      permissionToast.className = 'fixed top-4 right-4 bg-blue-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center space-x-2'
      permissionToast.innerHTML = '<span>📍</span> <span>Please allow location access...</span>'
      document.body.appendChild(permissionToast)
      
      // Detect device and get location using universal service
      const device = detectDevice()
      console.log(`📍 Requesting location from ${device.isMobile ? 'Mobile' : device.isDesktop ? 'Desktop' : 'Tablet'} device...`)
      ...
      getDeviceLocation({
        ...
        .then(async (location) => {
          ...
          if (currentUser.uid) {
            try {
              await shareLocation(currentUser.uid, { lat: location.lat, lng: location.lng })
        ...
        .catch(async (error) => {
            ...
            console.log('🔄 Attempting to fetch last known location from database...')
            try {
              const dbLocation = await getUserLocationFromDB(currentUser.uid)
              if (dbLocation) {
                console.log('✅ Using last known location from database:', dbLocation)
                ...
                toast.innerHTML = '<span>📍</span> <span>Using last known location from database</span>'
                document.body.appendChild(toast)
                setTimeout(() => toast.remove(), 3000)
                return
              }
            } catch (dbError) {
              console.error('❌ Failed to fetch location from database:', dbError)
            }
  ...
  useEffect(() => {
    if (!isNavigating || !selectedRoute || !userLocation || !selectedRoute.steps || selectedRoute.steps.length === 0) {
      return
    }
  ...
  useEffect(() => {
    ...
    // Detect device type
    const device = detectDevice()
    console.log(`🔄 Starting RELIABLE location tracking on ${device.isMobile ? 'Mobile' : device.isDesktop ? 'Desktop' : 'Tablet'} device...`)
    console.log(`   Device capabilities: GPS=${device.hasGPS}, HighAccuracy=${device.hasHighAccuracy}`)
    ...
    const updateLocationInDB = async (location, source = 'device') => {
      try {
        ...
        consecutiveErrors++
        console.error(`❌ Failed to update location in database (${source}):`, error)
        if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
          console.warn('⚠️ Too many consecutive errors, will retry with longer interval')
        }
        return false
      }
    }
    ...
    watchId = watchDeviceLocation(
      async (location) => {
        ...
        const shouldUpdateDB = distanceMoved > movementThreshold || 
                               !lastSavedLocation || 
                               !window.lastDBUpdate || 
        ...
        if (shouldUpdateDB) {
          ...
          if (!window.lastFirebaseUpdate || now - window.lastFirebaseUpdate > firebaseInterval) {
            window.lastFirebaseUpdate = now
            try {
              await shareLocation(user.uid, { lat: location.lat, lng: location.lng })
      ...
      (error) => {
        ...
        if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
          ...
          errorToast.className = 'fixed top-4 right-4 bg-red-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 text-sm'
    ...
    backupInterval = setInterval(async () => {
      try {
        ...
        const now = Date.now()
        if (!window.lastBackupUpdate || (now - window.lastBackupUpdate > updateIntervalMs * 3)) {
  ...
  const handleFindRoutes = async () => {
    if (!destination || !destination.trim()) {
      const toast = document.createElement('div')
      toast.className = 'fixed top-4 right-4 bg-yellow-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center space-x-2'
      toast.innerHTML = '<span>⚠️</span> <span>Please enter a destination</span>'
      document.body.appendChild(toast)
      setTimeout(() => toast.remove(), 3000)
      return
    }
    ...
    if (!userLocation) {
      const toast = document.createElement('div')
      toast.className = 'fixed top-4 right-4 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center space-x-2'
      toast.innerHTML = '<span>⚠️</span> <span>Please allow location access first</span>'
      document.body.appendChild(toast)
      setTimeout(() => toast.remove(), 3000)
      return
    }
    ...
    try {
      ...
      const fetchedRoutes = await getRoutesFromDirectionsAPI(userLocation, destCoords)
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-dark-bg">
        <div className="text-center">
          ...
          <p className="text-white text-lg font-semibold mb-2">Getting your location...</p>
          <p className="text-gray-400 text-sm">Please allow location access when prompted</p>
  ...
  return (
    <div className="flex h-screen bg-dark-bg overflow-hidden flex-col">
      {/* Top Section - Sidebar and Map */}
      <div className="flex flex-1 overflow-hidden">
        {/* Left Sidebar */}
        <div className="w-96 bg-dark-surface border-r border-gray-800 flex flex-col overflow-y-auto">
          {/* User Info Card */}
          <div className="p-6 border-b border-gray-800 sticky top-0 bg-dark-surface z-10">
            ...
            <div className="flex items-center justify-between mb-3 p-3 bg-dark-bg rounded-lg">
              ...
              <span className="text-neon-blue font-bold text-lg">{user?.credits || 250000}</span>
            </div>
              ...
              <div className="mb-3 p-3 bg-dark-bg rounded-lg border border-gray-700">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-gray-300 font-mono text-xs">{userLocation.lat.toFixed(6)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500 text-xs">Longitude:</span>
                    <span className="text-gray-300 font-mono text-xs">{userLocation.lng.toFixed(6)}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <button
            onClick={async () => {
              try {
                ...
                const toast = document.createElement('div')
                toast.className = 'fixed top-4 right-4 bg-blue-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center space-x-2'
                toast.innerHTML = '<span>👋</span> <span>Logged out successfully</span>'
                document.body.appendChild(toast)
                  ...
                  if (value.length > 2) {
                    try {
                      ...
                      if (results && results.length > 0) {
                        setDestinationSuggestions(results.map(r => ({
                          name: r.display_name,
                          lat: r.lat,
                          lng: r.lng
                        })))
                  ...
                  {destinationSuggestions.map((suggestion, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-400">{suggestion.name.split(',').slice(1).join(',').trim()}</div>
                      </div>
                    </button>
                  ))}
                </div>
      ...
      {showEmergencyContactSetup && (
        <EmergencyContactSetup 
          onClose={() => setShowEmergencyContactSetup(false)}
          onSuccess={() => setShowEmergencyContactSetup(false)}
        />
      )}
      </div>
    </div>
