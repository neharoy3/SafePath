...
const NavigationPanel = ({
  ...
  const getNavigationSteps = () => {
    ...
    return route.steps.map((step, index) => {
      ...
      const distanceKm = step.distance
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {route.distance} km • {route.eta} min
              </p>
            </div>
            <div
        ...
        <button
          onClick={async () => {
            ...
            reportModal.className =
              "fixed inset-0 bg-black/50 z-[3000] flex items-center justify-center";
            ...
            reportModal.querySelectorAll("button[data-type]").forEach((btn) => {
              btn.addEventListener("click", async () => {
                ...
                try {
                  if (!userLocation || !userLocation.lat || !userLocation.lng) {
                    alert(
                      "⚠️ Location not available. Please wait for GPS signal.",
                    );
                    reportModal.remove();
                    return;
                  }
                  ...
                  if (result.success) {
                    ...
                    const errorMsg =
                      result.error || result.data?.detail || "Unknown error";
                    alert(
